...
1.1.0   Upate to MS LogAnalytics API v2
...
$LogTables = @("DellDriverMissing_CL","DellDriverInstalled_CL","DellUpdateEvents_CL","DellUpdatePenetrationRate_CL","DellUpdateNonComplianceList_CL") # if you are using the Dell SA Dashboard, do not change this as the queries will no longer run successfully
#***********************************************************************************************************

## Do not change use for MS Eventlog ##
$MSEventLogName = "Dell"
...
function New-MSEventLog
    {
        ...
        $checkSource = [System.Diagnostics.EventLog]::SourceExists($MSEventSource)
        ...
        try
            {
                ...
                $evt=new-object System.Diagnostics.EventLog($MSEventLogName)
                $evt.Source=$MSEventSource
...
# test if SA File and folder exist
$CheckSA = Test-Path -Path $SAPath -ErrorAction SilentlyContinue
...
##### Upload the data to the API endpoint #####
# load Library
Add-Type -AssemblyName System.Web

# prepare the API transmit data
