...
$LogTypeDock = "DCSA_Dock"                                  # if you are using the Dell SA Dashboard, do not change this as the queries will no longer run successfully
$LogTypeMonitor= "DCSA_Monitor"                             # if you are using the Dell SA Dashboard, do not change this as the queries will no longer run successfully
$LogTypeWarranty= "DCSA_Warranty"                           # if you are using the Dell SA Dashboard, do not change this as the queries will no longer run successfully
...
# get latest filename from the ProgramData folder
try
    {
        $LastDSAReport = Get-ItemProperty -Path $env:ProgramData\Dell\DellDataVault\Archive\* -ErrorAction Stop | Sort-Object LastWriteTime, Name | Where-Object {$_.Name -notlike "*Alerts*"} | Select-Object -Last 1
    }
catch
    {
        Write-Host "No file Report exit"
        Exit 1
    }

# Get XML content from DSA Reports
try
    {
...
########################
[Array]$DockArray = $DSATelemetry.DCSA_Summary_Log.Hardware_Info.docks.Dock

foreach ($Dock in $DockArray)
    {
        # temporary array
        $TempDockArray = New-Object -TypeName psobject

        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'ComputerName' -Value $env:computername -Force
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'UserName' -Value $Username -Force
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'DeviceModel' -Value $Model -Force
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'SerialNo' -Value $ServiceTag -Force
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'AssetTag' -Value $AssetTag -Force
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'SystemSKU' -Value $DeviceSKU -Force
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'System_Service_Tag' -Value $DSATelemetry.DCSA_Summary_Log.Hardware_Info.System_Service_Tag
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'HardwareInfoGUID' -Value $Dock.HardwareInfoGUID
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'Docking_Time' -Value $Dock.Docking_Time
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'ServiceTag' -Value $Dock.ServiceTag
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'PackageFirmwareVersion' -Value $Dock.PackageFirmwareVersion
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'ModuleType' -Value $Dock.ModuleType
        $TempDockArray | Add-Member -MemberType NoteProperty -Name 'Name' -Value $Dock.MarketingName

        [Array]$DockList += $TempDockArray

    }

$DockListJSON = $DockList | ConvertTo-Json -Compress
...
$paramsDockListJSON = @{
    CustomerId = $customerId
    SharedKey  = $sharedKey
    Body       = ([System.Text.Encoding]::UTF8.GetBytes($DockListJSON))
    LogType    = $LogType
}
$LogResponse = Post-LogAnalyticsData @paramsDockListJSON


########################
# Data Monitor         #
########################
[Array]$MonitorArray = $DSATelemetry.DCSA_Summary_Log.Hardware_Info.Monitor

foreach ($Monitor in $MonitorArray)
    {
        # temporary array
        $TempMonitorArray = New-Object -TypeName psobject


        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'ComputerName' -Value $env:computername -Force
        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'UserName' -Value $Username -Force
        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'DeviceModel' -Value $Model -Force
        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'SerialNo' -Value $ServiceTag -Force
        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'AssetTag' -Value $AssetTag -Force
        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'SystemSKU' -Value $DeviceSKU -Force
        $TempMonitorArray | Add-Member -MemberType NoteProperty -Name 'System_Service_Tag' -Value $DSATelemetry.DCSA_Summary_Log.Hardware_Info.System_Service_Tag
        ...
        If ($DSATelemetry.DCSA_Summary_Log.Hardware_Info.System_Type -eq "Laptop(10)" -or $DSATelemetry.DCSA_Summary_Log.Hardware_Info.System_Type -eq "Laptop(31)" -or $DSATelemetry.DCSA_Summary_Log.Hardware_Info.System_Type -eq "Desktop(32)")
            ...
            If ($Monitor.Monitor_Type -eq "Non Dell Monitor")
                {
                    $TempMonitorArray| Add-Member -MemberType NoteProperty -Name 'Monitor_Type' -Value "Internal Display"
                }
            else
                {
                    $TempMonitorArray| Add-Member -MemberType NoteProperty -Name 'Monitor_Type' -Value $Monitor.Monitor_Type
                }
            }
        else
            {
        ...
        $TempMonitorArray| Add-Member -MemberType NoteProperty -Name 'Vendor_Specific_Data' -Value $Monitor.Vendor_Specific_Data
        $TempMonitorArray| Add-Member -MemberType NoteProperty -Name 'ModelName' -Value $Monitor.Model_Name
        $TempMonitorArray| Add-Member -MemberType NoteProperty -Name 'ServiceTag' -Value $Monitor.ServiceTag
        $TempMonitorArray| Add-Member -MemberType NoteProperty -Name 'DriverVersion' -Value $Monitor.DriverVersion

        [Array]$MonitorList += $TempMonitorArray

    }

$MonitorListJSON = $MonitorList | ConvertTo-Json -Compress

$LogType = $LogTypeMonitor

$paramsMonitorListJSON = @{
    CustomerId = $customerId
    SharedKey  = $sharedKey
    Body       = ([System.Text.Encoding]::UTF8.GetBytes($MonitorListJSON))
    LogType    = $LogType
}
$LogResponse = Post-LogAnalyticsData @paramsMonitorListJSON


########################
# Data Support         #
########################
$WarrantyWMI = Get-CimInstance -Namespace root/SupportAssist -ClassName DSA_SystemInformation

$WarrantyText = Switch ($WarrantyWMI.Entitlement)
    {
        0   {"Other"}
        1   {"Basic"}
        2   {"ProSupport"}
...
$WarrantyList = New-Object -TypeName psobject

$WarrantyList | Add-Member -MemberType NoteProperty -Name 'ComputerName' -Value $env:computername -Force
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'UserName' -Value $Username -Force
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'DeviceModel' -Value $Model -Force
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'SerialNo' -Value $ServiceTag -Force
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'AssetTag' -Value $AssetTag -Force
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'SystemSKU' -Value $DeviceSKU -Force
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'WarrantyLevel' -Value $WarrantyText
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'System_Service_Tag' -Value $DSATelemetry.DCSA_Summary_Log.Hardware_Info.System_Service_Tag
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'Entitlement ' -Value $WarrantyWMI.Entitlement
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'EntitlementExpiryDate' -Value $WarrantyWMI.EntitlementExpiryDate
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'IsConfigurationSet' -Value $WarrantyWMI.IsConfigurationSet
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'HostName' -Value $WarrantyWMI.Name
$WarrantyList | Add-Member -MemberType NoteProperty -Name 'SAVersion' -Value $WarrantyWMI.Version

$WarrantyListJSON = $WarrantyList | ConvertTo-Json -Compress

$LogType = $LogTypeWarranty

$paramsWarrantyListJSON = @{
    CustomerId = $customerId
    SharedKey  = $sharedKey
    Body       = ([System.Text.Encoding]::UTF8.GetBytes($WarrantyListJSON))
    LogType    = $LogType
}
$LogResponse = Post-LogAnalyticsData @paramsWarrantyListJSON